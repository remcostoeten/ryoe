...
export type TCreateEntityData<T extends TBaseEntity> = Omit<T, 'id' | 'createdAt' | 'updatedAt'>
export type TUpdateEntityData<T extends TBaseEntity> = Partial<Omit<T, 'id' | 'createdAt' | 'updatedAt'>>

function getCurrentTimestamp(): number {
	return Math.floor(Date.now() / 1000)
}

function buildWhereClause(filters: TFilterOptions): {
	sql: string
	args: any[]
} {
...
function buildLimitClause(pagination?: TPaginationOptions): {
	sql: string
	args: any[]
} {
...
export async function findMany<T extends TBaseEntity>(
	tableName: string,
	mapRowToEntity: (row: Record<string, unknown>) => T,
	options?: {
	...
	try {
		...
		if (options?.pagination) {
			...
			hasMore = total > (options.pagination.offset || 0) + entities.length
		} else {
			total = entities.length
			hasMore = false
		}

		return { success: true, data: entities, total, hasMore }
	} catch (error) {
		return {
			success: false,
			error: error instanceof Error ? error.message : 'Unknown error',
...
export async function create<T extends TBaseEntity>(
	...
	try {
		...
		const createdEntity = await findById(
			tableName,
