...
export async function updateTagWithValidation(
	id: number,
	data: TUpdateTagData
): Promise<TServiceResult<TTag>> {
	try {
		...
		if (data.name !== undefined) {
			...
			const existingTags = await tagRepository.searchTags(data.name.trim())
			const duplicateTag = existingTags.find(
				tag => tag.id !== id && tag.name.toLowerCase() === data.name!.trim().toLowerCase()
			)
...
export async function addTagToNote(
	noteId: number,
	tagId: number
): Promise<TServiceResult<TNoteTag>> {
	try {
