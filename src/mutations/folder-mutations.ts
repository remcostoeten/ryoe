	...
	return useMutation({
		...
		onError: (error, variables, context) => {
			...
			if (context?.previousFolder) {
				...
				updateFolderInParentCache(
					queryClient,
					context.previousFolder.parentId ?? null,
					variables.id,
...
export function useDeleteFolder(options?: TMutationOptions<boolean, TDeleteFolderVariables>) {
	const queryClient = useQueryClient()
	...
	return useMutation({
		...
		onError: (error, variables, context) => {
			...
			if (context?.folderToDelete) {
				addFolderToParentCache(
					queryClient,
					context.folderToDelete.parentId ?? null,
					context.folderToDelete
				)
...
export function useMoveFolder(options?: TMutationOptions<TFolderWithStats, TMoveFolderVariables>) {
	const queryClient = useQueryClient()
...
export function useReorderFolders(options?: TMutationOptions<boolean, TReorderFoldersVariables>) {
	const queryClient = useQueryClient()
