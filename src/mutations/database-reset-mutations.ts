...
export function useValidateReset(options?: TMutationOptions<boolean, void>) {
  return useMutation({
    ...
    onSuccess: (data) => {
      console.log('Reset validation completed:', data ? 'Reset successful' : 'Reset incomplete')
      options?.onSuccess?.(data, undefined)
    },
    onError: (error) => {
 ...
 */
export function useResetAndReload(options?: TMutationOptions<TDatabaseResetResult, { hardReset?: boolean }>) {
  const queryClient = useQueryClient()
