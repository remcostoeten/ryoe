...
import { RefreshCw, ExternalLink, Trash2, Activity, Network, AlertCircle } from 'lucide-react'
...
export function PortManagerDropdown({ className, maxHeight = '400px' }: TPortManagerDropdownProps) {
	const { isScanning, error, scanPorts, killPort, getDevelopmentPorts } = usePortManager({
	...
	const activePorts = developmentPorts.filter(
		...
		return (
			<Card className={cn('w-80', className)}>
				<CardContent className='p-4'>
					...
					<Button variant='outline' size='sm' onClick={scanPorts} className='mt-2 w-full'>
		...
		<Card className={cn('w-80', className)}>
			...
			<CardContent className='p-0'>
				{developmentPorts.length === 0 ? (
					...
					<div className='space-y-1 max-h-80 overflow-y-auto' style={{ maxHeight }}>
