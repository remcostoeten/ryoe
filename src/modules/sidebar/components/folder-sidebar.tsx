...
type OptimisticFolderAction =
  | { type: 'CREATE_FOLDER'; parentId: number | null; tempId: string; name: string }
  | { type: 'DELETE_FOLDER'; folderId: number }
  | { type: 'RENAME_FOLDER'; folderId: number; newName: string }
  | { type: 'MOVE_FOLDER'; folderId: number; newParentId: number | null; newPosition: number }
  | { type: 'CREATE_NOTE'; folderId: number; tempId: string; note: TNote }
  | { type: 'DELETE_NOTE'; noteId: number; folderId: number }
  | { type: 'UPDATE_NOTE'; noteId: number; folderId: number; updates: Partial<TNote> }
...
const CustomContextMenu = ({
  ...
  }, [contextMenu.visible, onClose, onEdit, onCreateChild, onCreateNote, onToggleFavorite, showNotes])
...
export function FolderSidebar({
  ...
  const [optimisticActions, addOptimisticAction] = useOptimistic<OptimisticFolderAction[], OptimisticFolderAction>(
    [],
  ...
  function addToParentFolder(folders: any[], parentId: number, newFolder: any): any[] {
    return folders.map(folder => {
      ...
      if (folder.children) {
        return { ...folder, children: addToParentFolder(folder.children, parentId, newFolder) }
      }
      return folder
    })
  }
  ...
  }, [createFolderFromContext, expandedFolderIds, toggleFolder, startEditing, addOptimisticAction])
  ...
  }, [addOptimisticAction, expandedFolderIds, toggleFolder, addNoteToFolder, removeNoteFromFolder, navigate, onNoteSelect])
  ...
  const handleDeleteNote = useCallback(async (note: TNote) => {
  ...
  }, [deleteNoteMutation, removeNoteFromFolder, addNoteToFolder, selectedNoteId, navigate, addOptimisticAction])
  ...
  const handleUpdateNote = useCallback(async (note: TNote, newTitle: string) => {
  ...
  const handleDragStart = useCallback((e: React.DragEvent, folder: any) => {
    ...
    dragImage.className = 'bg-card border border-border rounded-lg px-3 py-2 text-sm shadow-lg'
  ...
  const handleDragOver = useCallback((e: React.DragEvent, targetFolder: any) => {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent, targetFolder: any) => {
    e.preventDefault()
  ...
  const renderFolder = useCallback((folder: any, level = 0) => {
      ...
      <div key={folder.id || folder.tempId}>
              ...
              >
                <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
                  ...
                  <FolderIcon className={cn(
                    ...
                    isSelected ? "text-accent-foreground/80" : "text-muted-foreground/70"
                  )} />
        ...
        {showNotes && isExpanded && (() => {
          ...
          return folderNotes.length > 0 && (
            <div className="space-y-1 animate-in slide-in-from-top-2 duration-200" style={{ paddingLeft: `${indentLevel + 24}px` }}>
                ...
                <NoteItem
                  ...
                  onDuplicate={async (note) => {
                    ...
                    try {
                      ...
                      if (response.success && response.data) {
                        removeNoteFromFolder(note.folderId, duplicatedNote.id)
    ...
    expandedFolderIds, selectedFolderId, editingFolderId, dragState, showNotes,
    getNotesForFolder, selectedNoteId, editingNoteId, editingName,
    toggleFolder, selectFolder, startEditing, handleRightClick, handleDragStart,
    handleDragEnd, handleDragOver, handleDragLeave, handleDrop, handleRenameFolder,
    stopEditing, navigate, onNoteSelect, handleUpdateNote, handleDeleteNote,
    addNoteToFolder, removeNoteFromFolder, toggleNoteFavoriteMutation, enableDragDrop
  ])
