...
import type { TFolder, TNote, TWorkspaceItem, TWorkspaceStats } from '@/domain/entities/workspace'
import type {
	IFolderRepository,
	INoteRepository,
...
export function useWorkspaceFeature(
	...
	const queries: TWorkspaceQueries = useMemo(
		() => ({
			...
			getNotesForFolder: folderId => state.notes.filter(note => note.folderId === folderId),

			getFavorites: () => [...state.folders, ...state.notes].filter(item => item.isFavorite),

			getPublicItems: () => [...state.folders, ...state.notes].filter(item => item.isPublic),
