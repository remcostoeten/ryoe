...
function categorizeDependency(name: string, isDev = false): TechStackCategory {
	...
	if (
		['react', 'react-dom', 'react-router', 'react-hook-form', 'react-error-boundary'].includes(
			name
		)
	) {
		return 'frontend'
	...
	if (
		...
		['framer-motion', 'motion', 'class-variance-authority', 'clsx', 'tailwind-merge'].includes(
			name
		)
	) {
		return 'ui'
	...
	// Build tools
	if (
		['vite', 'typescript', 'rollup', '@vitejs/plugin-react', '@mdx-js/rollup'].includes(name) ||
		name.includes('vite') ||
	...
	if (
		isDev ||
		['eslint', 'prettier', 'tsx', 'drizzle-kit', '@types/', 'autoprefixer'].some(tool =>
			name.includes(tool)
		)
	) {
		return 'dev'
	...
	// Testing
	if (['vitest', 'jest', '@testing-library/', 'cypress'].some(tool => name.includes(tool))) {
		return 'testing'
	}
...
export function getTechStack(packageInfo: PackageInfo): TechStackItem[] {
	const allDeps = [
		...
		...Object.entries(packageInfo.devDependencies || {}).map(
...
export function readPackageJson(): Promise<PackageInfo> {
	...
	return Promise.resolve({
		...
		license: 'MIT',
		keywords: ['shadcn', 'tauri', 'rust', 'vite', 'react', 'tailwindcss', 'ui', 'desktop'],
		dependencies: {
