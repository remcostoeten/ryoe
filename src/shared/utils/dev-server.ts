...
export function setupHMRErrorHandling(): void {
	...
	window.WebSocket = class extends originalWebSocket {
		constructor(url: string | URL, protocols?: string | string[]) {
			...
			this.addEventListener('error', event => {
				...
				if (url.toString().includes('vite') || url.toString().includes('hmr')) {
					console.warn(
			...
			this.addEventListener('close', () => {
				...
				if (url.toString().includes('vite') || url.toString().includes('hmr')) {
					console.warn('HMR WebSocket closed (will attempt to reconnect)')
				}
			})
		}
	}
