...
import { openMdxDirectoryPicker, validateMdxPath } from '@/shared/utils/file-picker'
...
export function DirectoryPicker({
	...
	return (
		<div className={cn('space-y-2', className)}>
			...
			{displayError && <p className='text-sm text-red-400 mt-1'>{displayError}</p>}

			<p className='text-xs text-gray-500'>
				Choose where to store your MDX files. You can type a path or click Browse to select
				a directory.
			</p>
		</div>
	)
...
export function OnboardingDirectoryPicker({
	...
	return (
		<div className={cn('space-y-3', className)}>
			{label && (
				<label className='block text-white/80 text-sm font-medium text-left'>{label}</label>
			)}
			...
			{displayError && <p className='text-sm text-red-400/80 mt-1'>{displayError}</p>}
		</div>
	)
}
