  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.parentId) {
        ...
        queryClient.invalidateQueries({ queryKey: FOLDERS_QUERY_KEYS.ROOT_FOLDERS })
      }
      
      // Invalidate hierarchy
      queryClient.invalidateQueries({ queryKey: FOLDERS_QUERY_KEYS.FOLDER_HIERARCHY })
    }
  })
}
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.isFavorite !== undefined) {
        queryClient.invalidateQueries({ queryKey: FOLDERS_QUERY_KEYS.FAVORITE_FOLDERS })
      }
    }
  })
}
...
export function useDeleteFolder() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      // Remove from cache
      queryClient.removeQueries({ queryKey: FOLDERS_QUERY_KEYS.FOLDER_BY_ID(variables.id) })
...
export function useMoveFolder() {
  ...
  return useMutation({
    mutationFn: ({ id, newParentId, newPosition }: { id: number; newParentId: number | null; newPosition?: number }) => 
...
// Cache utilities
export function getFolderFromCache(queryClient: ReturnType<typeof useQueryClient>, id: number): TFolderWithStats | undefined {
  return queryClient.getQueryData(FOLDERS_QUERY_KEYS.FOLDER_BY_ID(id))
}

export function setFolderCache(queryClient: ReturnType<typeof useQueryClient>, folder: TFolderWithStats) {
  queryClient.setQueryData(FOLDERS_QUERY_KEYS.FOLDER_BY_ID(folder.id), folder)
}

export function invalidateFolderQueries(queryClient: ReturnType<typeof useQueryClient>, parentId?: number | null) {
  if (parentId !== undefined) {
    if (parentId) {
      queryClient.invalidateQueries({ queryKey: FOLDERS_QUERY_KEYS.CHILD_FOLDERS(parentId) })
    } else {
      queryClient.invalidateQueries({ queryKey: FOLDERS_QUERY_KEYS.ROOT_FOLDERS })
    }
  }
