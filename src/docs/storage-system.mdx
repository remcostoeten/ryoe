...
```typescript
async function createTauriStoreProvider(storeName: string): Promise<TStorageProvider> {
...
```typescript
async function createFallbackProvider(storeName: string): Promise<TStorageProvider> {
	...
	return {
		...
		async clear(): Promise<void> {
			try {
				...
				console.warn('Falling back to localStorage for clearing storage:', error)
...
console.log(`Window: ${size.width}x${size.height} at (${position.x}, ${position.y})`)
```
