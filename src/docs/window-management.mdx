...
The Notr application includes advanced window management features that automatically save and restore window position, size, and state across application restarts. This provides a seamless user experience by remembering where you positioned your application window.
...
```bash
~/.local/share/com.remcostoeten.notr/config/window-state.dat
```
...
```bash
~/.local/share/com.remcostoeten.notr/config/
```
