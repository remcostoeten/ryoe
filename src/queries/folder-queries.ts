...
export function useFolder(id: number, options?: TQueryOptions<TFolderWithStats>) {
	return useQuery({
...
export function useChildFolders(parentId: number, options?: TQueryOptions<TFolderWithStats[]>) {
	return useQuery({
...
export function useFolderPath(id: number, options?: TQueryOptions<TFolderWithStats[]>) {
	return useQuery({
...
export function invalidateFolderQueries(queryClient: any, folderId?: number, parentId?: number) {
	if (folderId) {
		...
		queryClient.invalidateQueries({
			queryKey: QUERY_KEYS.FOLDER_PATH(folderId),
		})
	}
	if (parentId) {
		queryClient.invalidateQueries({
...
export function getChildFoldersFromCache(
	queryClient: any,
	parentId: number
): TFolderWithStats[] | undefined {
...
export function moveFolderBetweenParentsCache(
	...
	addFolderToParentCache(queryClient, newParentId, {
		...updatedFolder,
		parentId: updatedFolder.parentId || undefined,
	})

	// Update individual folder cache
	setFolderCache(queryClient, folder.id, {
		...updatedFolder,
		parentId: updatedFolder.parentId || undefined,
	})
}
