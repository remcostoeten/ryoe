...
import { getNoteById, getNotesByFolder, searchNotesWithOptions } from '@/api/services/notes-service'
...
export function useNote(id: number, options?: TQueryOptions<TNoteWithMetadata>) {
	return useQuery({
...
export function useSearchNotes(
	...
	return useQuery({
		...
		enabled:
			!!searchOptions.query && searchOptions.query.length >= 2 && options?.enabled !== false,
...
export function setNotesByFolderCache(
	queryClient: any,
	folderId: number | null,
	data: TNoteWithMetadata[]
) {
...
export function getNotesByFolderFromCache(
	queryClient: any,
	folderId: number | null
): TNoteWithMetadata[] | undefined {
...
export function updateNoteInFolderCache(
	...
	if (notes) {
		const updatedNotes = notes.map(note => (note.id === noteId ? updater(note) : note))
...
export function moveNoteBetweenFoldersCache(
	...
	addNoteToFolderCache(queryClient, newFolderId, {
		...updatedNote,
		folderId: updatedNote.folderId || undefined,
	})

	// Update individual note cache
	setNoteCache(queryClient, note.id, {
		...updatedNote,
		folderId: updatedNote.folderId || undefined,
	})
}
